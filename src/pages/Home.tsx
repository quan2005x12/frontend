...
import { Camera, ChevronRight, Verified, Sparkles, X, UploadCloud } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Link, useNavigate } from 'react-router-dom';
import { type DishMatch } from '../types';

// TODO: Fetch from backend later
const recentCurations: DishMatch[] = [];
    ...
    <div className="space-y-16">
      ...
      >
        ...
        <div className="relative group">
              ...
              {selectedImage ? (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-white space-y-4">
                    ...
                    <p className="font-bold tracking-widest uppercase text-xs">Phóng ảnh để đổi</p>
                  </div>
          ...
          {/* Decorative Mortar */}
          <div className="absolute -bottom-12 -right-6 hidden lg:block z-10">
            <motion.div 
              whileHover={{ rotate: 5, scale: 1.05 }}
              className="w-40 h-40 rounded-3xl rotate-12 overflow-hidden shadow-2xl border-8 border-surface"
            >
              <img 
                src="https://picsum.photos/seed/culinary-art/400/400" 
                alt="Herbs" 
      ...
      <section className="space-y-12">
        ...
        <div className="flex gap-8 overflow-x-auto pb-6 no-scrollbar snap-x">
          {recentCurations.length > 0 ? recentCurations.map((item, index) => (
            <motion.div 
              key={index}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Link to="/detail" className="block space-y-6 text-center">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <h3 className="text-2xl font-bold text-on-surface group-hover:text-primary transition-colors">{item.name}</h3>
                    <p className="text-sm font-serif italic text-on-surface-variant">Thành phần tươi ngon, đậm đà</p>
                  </div>
                </div>
              </Link>
            </motion.div>
        ...
        <div className="flex justify-center">
          <button className="px-12 py-4 border border-on-surface/10 rounded-full font-bold uppercase text-[10px] tracking-[0.2em] hover:bg-on-surface hover:text-white transition-all">
